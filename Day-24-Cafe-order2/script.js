...
function displayOrders() {
    ...
    for (let i = 0; i < cafeOrders.length; i++) {
      ...
      <div class="item">
        ...
        <button onclick="completeOrder(${i})">
         ...
         <button onclick="orderInProgress(${i})">
        Order in progress
        </button>
    ...
    <p class="subtitle">Cafe One order</p>
    ${output}
    `;
}

function orderInProgress(i) {
    cafeOrders[i].status = "In Progress";
    displayOrders();
}
function completeOrder(i) {
    cafeOrders[i].status = "Completed";
    displayOrders();
}
